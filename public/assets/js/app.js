            ...
            try {
                ...
                icons.forEach(function(iconEl) {
                    ...
                    if (iconName && feather.icons && feather.icons[iconName]) {
                        try {
                            const svg = feather.icons[iconName].toSvg({class: iconEl.className});
                            const temp = document.createElement('div');
                            temp.innerHTML = svg;
                            if (temp.firstChild && iconEl.parentNode) {
                                iconEl.parentNode.replaceChild(temp.firstChild, iconEl);
                            }
                        } catch(e) {
                            // Skip this icon
                        }
                ...
                // Fallback to normal replace if available
                try {
                    feather.replace();
                } catch(e2) {
                    // Silent fail
