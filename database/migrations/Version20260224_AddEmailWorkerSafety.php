...
final class Version20260224_AddEmailWorkerSafety extends AbstractMigration
{
    ...
    {
        return 'Email worker güvenliği için claim lock alanları, pool cursor ve smtp usage event tablosu ekler';
    }
    ...
    public function up(Schema $schema): void
    {
        ...
        if (!$hasColumn('email_orders', 'locked_at')) {
            $this->addSql("ALTER TABLE email_orders ADD COLUMN locked_at DATETIME NULL DEFAULT NULL COMMENT 'Worker claim lock zamanı'");
        }
        ...
        if (!$hasColumn('email_orders', 'attempt_count')) {
            $this->addSql("ALTER TABLE email_orders ADD COLUMN attempt_count INT NOT NULL DEFAULT 0 COMMENT 'Worker claim deneme sayısı'");
        }
